...
{
    public class LoaiTonService : ILoaiTonService
    {
        private readonly ESaleDbContext _context;
        private readonly IStorageService _storageService;
        private const string USER_CONTENT_FOLDER_NAME = "user-content";

        public LoaiTonService(ESaleDbContext context, IStorageService storageService)
        {
            _context = context;
            _storageService = storageService;
        }

        public async Task<int> Create(LoaiTonCreateRequest request)
        {
            var item = new DanhMucLoaiTon()
            {
                MaSo = request.MaSo,
            ...
            _context.DanhMucLoaiTons.Add(item);
        ...
        {
            var item = await _context.DanhMucLoaiTons.FindAsync(Id);

            if (item == null) throw new ESaleException($"Không tìm thấy đối tượng: {Id}");

            _context.DanhMucLoaiTons.Remove(item);

            return await _context.SaveChangesAsync();
        }

        public async Task<PagedResult<LoaiTonVm>> GetAllPaging(GetLoaiTonPagingRequest request)
        {
            //1. Select join
            var query = from p in _context.DanhMucLoaiTons
                        select new { p };
            ...
            var pagedResult = new PagedResult<LoaiTonVm>()
            {
        ...
        public async Task<LoaiTonVm> GetById(int Id)
        {
            var doday = await _context.DanhMucLoaiTons.FindAsync(Id);

            var dodayViewModel = new LoaiTonVm()
            {
                MaSo = doday.MaSo,
        ...
        public async Task<int> Update(LoaiTonUpdateRequest request)
        {
            var item = await _context.DanhMucLoaiTons.FindAsync(request.Id);
