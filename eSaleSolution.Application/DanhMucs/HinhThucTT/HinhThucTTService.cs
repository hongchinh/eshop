...
namespace eSaleSolution.Application.DanhMuc.HinhThucTTs
{
    public class HinhThucTTService : IHinhThucTTService
    {
        ...
        public HinhThucTTService(ESaleDbContext context, IStorageService storageService)
        {
            _context = context;
            _storageService = storageService;
        }

        public async Task<int> Create(HinhThucTTCreateRequest request)
        {
            var item = new DanhMucHinhThucTT()
            {
                MaSo = request.MaSo,
            ...
            _context.DanhMucHinhThucTTs.Add(item);
        ...
        {
            var item = await _context.DanhMucHinhThucTTs.FindAsync(Id);

            if (item == null) throw new ESaleException($"Không tìm thấy đối tượng: {Id}");

            _context.DanhMucHinhThucTTs.Remove(item);

            return await _context.SaveChangesAsync();
        }

        public async Task<PagedResult<HinhThucTTVm>> GetAllPaging(GetHinhThucTTPagingRequest request)
        {
            //1. Select join
            var query = from p in _context.DanhMucHinhThucTTs
                        select new { p };
            ...
            var pagedResult = new PagedResult<HinhThucTTVm>()
            {
        ...
        public async Task<HinhThucTTVm> GetById(int Id)
        {
            var doday = await _context.DanhMucHinhThucTTs.FindAsync(Id);

            var dodayViewModel = new HinhThucTTVm()
            {
                MaSo = doday.MaSo,
        ...
        public async Task<int> Update(HinhThucTTUpdateRequest request)
        {
            var item = await _context.DanhMucHinhThucTTs.FindAsync(request.Id);
