...
namespace eSaleSolution.Application.DanhMuc.KhoanChis
{
    public class KhoanChiService : IKhoanChiService
    {
        ...
        public KhoanChiService(ESaleDbContext context, IStorageService storageService)
        {
            _context = context;
            _storageService = storageService;
        }

        public async Task<int> Create(KhoanChiCreateRequest request)
        {
            var item = new DanhMucKhoanChi()
            {
                MaSo = request.MaSo,
            ...
            _context.DanhMucKhoanChis.Add(item);
        ...
        {
            var item = await _context.DanhMucKhoanChis.FindAsync(Id);

            if (item == null) throw new ESaleException($"Không tìm thấy đối tượng: {Id}");

            _context.DanhMucKhoanChis.Remove(item);

            return await _context.SaveChangesAsync();
        }

        public async Task<PagedResult<KhoanChiVm>> GetAllPaging(GetKhoanChiPagingRequest request)
        {
            //1. Select join
            var query = from p in _context.DanhMucKhoanChis
                        select new { p };
            ...
            var pagedResult = new PagedResult<KhoanChiVm>()
            {
        ...
        public async Task<KhoanChiVm> GetById(int Id)
        {
            var doday = await _context.DanhMucKhoanChis.FindAsync(Id);

            var dodayViewModel = new KhoanChiVm()
            {
                MaSo = doday.MaSo,
        ...
        public async Task<int> Update(KhoanChiUpdateRequest request)
        {
            var item = await _context.DanhMucKhoanChis.FindAsync(request.Id);
